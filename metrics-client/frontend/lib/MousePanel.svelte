<script>
  ...
  globalThis.runtime.EventsOn("mouseCollectorConnected", (state) => {
    connected = state;
    console.log("connected to mouse collector");
  });

  globalThis.runtime.EventsOn("mouseCollectorReceiveData", (data) => {
    console.log(
      "recieved data from mouse collector: " + JSON.stringify(data.Data[0].Name)
    );
...
>
  ...
  <div class="bar flex items-center gap-2">
    ...
    {#if !connected}
      <GradientButton
        color="green"
        on:click={() => {
          Connect("mouse", host, port, updateMethodKeys[updateMethod]);
        }}
      ...
      >
    {:else}
      <GradientButton color="red" on:click={() => Disconnect("mouse")}
    ...
    <GradientButton color="blue" on:click={() => GetDataManually("mouse")}
  ...
  {#if response?.Status && response?.Status != "error"}
    <ul class="text-left">
      {#each response?.Data as mouse}
        <li class="flex gap-1 mt-4">
          <HugeiconsMouse03 class="" />
          Name: {mouse.Name}
        </li>
        <ul class="flex-col">
          <li class="flex gap-1">
            <LucideMousePointerClick /> Number of buttons: {mouse.BtnCount}
          </li>
          <li class="flex gap-1">
            <LineMdDoubleArrowVertical /> Vertical scroll wheel: {mouse.VWheel
              ? "есть"
              : "нет"}
          </li>
          <li class="flex gap-1">
            <LineMdDoubleArrowHorizontal /> Horizontal scroll wheel: {mouse.HWheel
              ? "есть"
              : "нет"}
          </li>
        </ul>
